...
import "@openzeppelin/contracts@v4.9.0/utils/cryptography/ECDSA.sol";
...
abstract contract OnChainMetadata {
    ...
    // Individual functions to update each trait
    function _updateTokenName(uint256 _tokenId) internal {
        _setValue(
            _tokenId,
            key_token_name,
            abi.encode(string.concat("AEGAEGAEG #", Strings.toString(_tokenId)))
        );
    }

    function _updateTokenDescription(uint256 _tokenId) internal {
        _setValue(
            _tokenId,
            key_token_description,
            abi.encode("AEGAEGAEGAEGAEGAEGAEGAEGAEG")
        );
    }

    function _updateTokenImage(uint256 _tokenId) internal {
        _setValue(
            _tokenId,
            key_token_image,
            abi.encode("ipfs://QmPbxeGcXhYQQNgsC6a36dDyYUcHgMLnGKnF8pVFmGsvqi")
        );
    }

    function _updateEnemyKill(uint256 _tokenId, uint256 _enemyKill) internal {
        bytes[] memory trait_types = _getValues(
            _tokenId,
            key_token_attributes_trait_type
        );
        bytes[] memory trait_values = _getValues(
            _tokenId,
            key_token_attributes_trait_value
        );
        bytes[] memory trait_display = _getValues(
        ...
        if (trait_types.length < 7) {
            trait_types = new bytes[](7);
            trait_values = new bytes[](7);
            trait_display = new bytes[](7);
        }

        trait_types[0] = abi.encode("Z1");
        trait_values[0] = abi.encode(Strings.toString(_enemyKill));
        trait_display[0] = abi.encode("");

        _setValues(_tokenId, key_token_attributes_trait_type, trait_types);
        _setValues(_tokenId, key_token_attributes_trait_value, trait_values);
        _setValues(_tokenId, key_token_attributes_display_type, trait_display);
    }

    function _updateDamageDealt(
        uint256 _tokenId,
        ...
        trait_types[1] = abi.encode("Z2");
        trait_values[1] = abi.encode(Strings.toString(_damageDealt));
    ...
    function _updateDamageTaken(
        ...
        trait_types[2] = abi.encode("Z3");
        trait_values[2] = abi.encode(Strings.toString(_damageTaken));
    ...
    function _updatePlayerDied(uint256 _tokenId, uint256 _playerDied) internal {
        bytes[] memory trait_types = _getValues(
            _tokenId,
            key_token_attributes_trait_type
        );
        ...
        trait_types[3] = abi.encode("Z4");
        trait_values[3] = abi.encode(Strings.toString(_playerDied));
    ...
    function _updatePlayerRevived(
        ...
        trait_types[4] = abi.encode("Z5");
        trait_values[4] = abi.encode(Strings.toString(_playerRevived));
    ...
    function _updateMissionCompleted(
        uint256 _tokenId,
        ...
        trait_types[5] = abi.encode("Z6");
        trait_values[5] = abi.encode(Strings.toString(_missionCompleted));
    ...
    function _updateKeystoneCompleted(
        uint256 _tokenId,
        ...
        trait_types[6] = abi.encode("Z7");
        trait_values[6] = abi.encode(Strings.toString(_keystoneCompleted));
